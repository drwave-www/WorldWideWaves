 ...
 *
 * Tests cover:
 * - Real ButtonWave component functionality with actual business logic
 * - Component behavior under different event states
...
class CommonComponentsTest : BaseComponentTest() {
    ...
    @OptIn(ExperimentalTime::class)
    @Test
    fun buttonWave_soonEventInArea_isEnabled() {
        // Test ButtonWave with SOON event status
        val navigator = WaveNavigator { }

        setThemedContent {
            ButtonWave(
                eventId = "test-soon-event",
                eventState = Status.SOON,
                endDateTime = null,
                isInArea = true,
                onNavigateToWave = navigator,
                modifier = Modifier.testTag("button-wave-soon")
            )
        }

        // Verify button is enabled for SOON events when in area
        composeTestRule.onNodeWithTag("button-wave-soon").assertIsDisplayed()
        composeTestRule.onNodeWithTag("button-wave-soon").assertIsEnabled()
    }
    ...
    @OptIn(ExperimentalTime::class)
    @Test
    fun buttonWave_waitingEvent_isDisabled() {
        // Test ButtonWave with WAITING event status
        val navigator = WaveNavigator { }
        ...
        setThemedContent {
            ButtonWave(
                ...
                isInArea = true,
                onNavigateToWave = navigator,
                modifier = Modifier.testTag("button-wave-waiting")
            )
        }

        // Verify button is disabled for WAITING events
        composeTestRule.onNodeWithTag("button-wave-waiting").assertIsDisplayed()
        composeTestRule.onNodeWithTag("button-wave-waiting").assertIsNotEnabled()
    }
}