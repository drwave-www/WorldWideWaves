...
countries, culminating in a global wave. The project aims to transcend physical and cultural boundaries, 
fostering unity, community, and shared human experience by leveraging real-time coordination and location-based services.
...
This script sets up the environment and generates map tiles for a specified event. It performs the following steps:
...
This script downloads OpenStreetMap (OSM) data for specified events and extracts the bounding box (BBOX) for each event. It performs the following steps:
...
This script generates MBTiles files from the downloaded OSM data for each event. It calls `generate_map.dep.sh` for each event to perform the map generation process.
