...
class BaseMapDownloadViewModelTest {
    private class TestMapDownloadViewModel(
        ...
        override suspend fun isMapInstalled(mapId: String): Boolean = mockMapInstalled
        ...
        override fun getLocalizedErrorMessage(errorCode: Int): String = "$mockErrorMessage (code: $errorCode)"
    ...
    @Test
    fun `checkIfMapIsAvailable sets NotAvailable when map not installed`() =
        runTest {
    ...
    @Test
    fun `checkIfMapIsAvailable triggers download when autoDownload true`() =
        runTest {
            val viewModel = TestMapDownloadViewModel(mockMapInstalled = false)
    ...
    @Test
    fun `downloadMap succeeds and sets Installed state`() =
        runTest {
    ...
    @Test
    fun `downloadMap failure sets Failed state`() =
        runTest {
    ...
    @Test
    fun `cancelDownload calls platform cancellation`() =
        runTest {
            val viewModel = TestMapDownloadViewModel()
    ...
    @Test
    fun `handleDownloadProgress sets correct progress`() =
        runTest {
            val viewModel = TestMapDownloadViewModel()
    ...
    @Test
    fun `handleDownloadSuccess sets Installed state and resets retry`() =
        runTest {
            val viewModel = TestMapDownloadViewModel()
    ...
    @Test
    fun `handleDownloadFailure with retry sets Retrying state`() =
        runTest {
            val viewModel = TestMapDownloadViewModel()
    ...
    @Test
    fun `handleDownloadFailure without retry sets Failed state`() =
        runTest {
            val viewModel = TestMapDownloadViewModel()
    ...
    @Test
    fun `handleDownloadCancellation sets NotAvailable and resets retry`() =
        runTest {
            val viewModel = TestMapDownloadViewModel()
    ...
    @Test
    fun `retry exhaustion leads to failed state`() =
        runTest {
            val viewModel = TestMapDownloadViewModel()
    ...
    @Test
    fun `handleInstallComplete calls clearCache and sets success`() =
        runTest {
            val viewModel = TestMapDownloadViewModel()
