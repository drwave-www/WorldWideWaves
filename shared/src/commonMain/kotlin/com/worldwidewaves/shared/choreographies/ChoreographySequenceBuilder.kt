...
/**
 * Builds visual choreography sequences for different phases of wave events.
 *
 ...
 *
 * The builder handles sequence transitions, looping, and time-based frame selection
 * to create a synchronized visual experience across all wave phases.
 */
@OptIn(ExperimentalTime::class)
open class ChoreographySequenceBuilder<T> : KoinComponent {
        ...
        val resolvedImage =
            resolvedImageCache.getOrPut(imageKey) {
                try {
                    ...
                    Log.w("ChoreographySequenceBuilder", "Failed to resolve image for $imageKey: ${e.message}")
                    null
                }
            }
    ...
    private suspend fun loadDefinition(definitionResource: String): ChoreographyDefinition {
        ...
        val loadedDefinition =
            try {
                ...
                Log.e("ChoreographySequenceBuilder", "Error loading choreography definition: ${e.message}")
    ...
    open suspend fun getCurrentWarmingSequence(startTime: Instant): DisplayableSequence<T>? {
        ...
        Log.v("ChoreographySequenceBuilder", "wrappedElapsedTime: ${wrappedElapsedTime.inWholeSeconds} seconds")
    ...
    open fun getHitSequenceImmediate(): DisplayableSequence<T>? {
        val resolved = resolvedSequences
        if (resolved == null) {
            Log.w("ChoreographySequenceBuilder", "Hit sequence requested but choreography not pre-loaded! This may cause timing issues.")
        }
