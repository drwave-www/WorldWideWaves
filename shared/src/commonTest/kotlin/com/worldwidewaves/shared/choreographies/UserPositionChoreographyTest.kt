...
class UserPositionChoreographyTest : KoinTest {
     ...
     */
    @Test
    fun `should track user position changes during wave events`() =
        runTest {
     ...
     */
    @Test
    fun `should verify choreography step timing and transitions`() =
        runTest {
            ...
            val warmingSequence1 =
                createTestChoreographySequence(
                    frameCount = 3,
     ...
     */
    @Test
    fun `should test position-based choreography triggers`() =
        runTest {
            if (CIEnvironment.isCI) {
     ...
     */
    @Test
    fun `should meet choreography timing accuracy requirements`() =
        runTest {
            ...
            repeat(iterations) { iteration ->
                ...
                val wrappedTime =
                    if (sequenceDuration.isPositive()) {
            ...
            val p95Time =
                timingMeasurements.sorted().let { sorted ->
            ...
            assertTrue(
                ...
                "Average timing calculation (${String.format(
                    "%.2f",
                    averageTime,
                )}ms) should be within tolerance (${CHOREOGRAPHY_TIMING_TOLERANCE_MS}ms)",
            )
     ...
     */
    @Test
    fun `should integrate position tracking with choreography response`() =
        runTest {
            if (CIEnvironment.isCI) {
            ...
            positionScenario.forEachIndexed { index, (position, inArea, expectedPhase) ->
                ...
                println(
                    "✅ Integration phase $expectedPhase: Position (${position.lat}, ${position.lng}), InArea: $inArea, Time: ${elapsedTime.inWholeSeconds}s",
                )
            }
