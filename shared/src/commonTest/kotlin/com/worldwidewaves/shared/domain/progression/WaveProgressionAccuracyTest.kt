...
class WaveProgressionAccuracyTest {
     ...
     */
    @Test
    fun `should calculate wave progression accurately`() =
        runTest {
     ...
     */
    @Test
    fun `should maintain monotonic progression throughout wave lifecycle`() =
        runTest {
            val mockClock = mockk<IClock>()
     ...
     */
    @Test
    fun `should meet performance requirements for progression calculation`() =
        runTest {
            val mockClock = mockk<IClock>()
            val baseTime = Instant.fromEpochSeconds(1640995200)
            val tracker = DefaultWaveProgressionTracker(mockClock)
     ...
     */
    @Test
    fun `should handle position-based progression correctly`() =
        runTest {
            if (CIEnvironment.isCI) {
