...
class CompleteEventObservationTest : KoinTest {
     ...
     */
    @Test
    fun `should test full event observation workflow from start to finish`() =
        runTest {
     ...
     */
    @Test
    fun `should verify observer lifecycle management across all states`() =
        runTest {
     ...
     */
    @Test
    fun `should test concurrent event observation scenarios with resource management`() =
        runTest {
            if (CIEnvironment.isCI) {
     ...
     */
    @Test
    fun `should handle stress tests with multiple simultaneous events`() =
        runTest {
            if (CIEnvironment.isCI) {
            ...
            val stressEvents =
                (1..STRESS_TEST_EVENT_COUNT).map { index ->
     ...
     */
    @Test
    fun `should maintain observer state consistency during complex scenarios`() =
        runTest {
     ...
     */
    @Test
    fun `should manage resources and memory cleanup properly during observations`() =
        runTest {
