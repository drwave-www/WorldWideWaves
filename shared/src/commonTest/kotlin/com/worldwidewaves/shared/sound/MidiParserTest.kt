...
class MidiParserTest {
    ...
    fun `should throw exception for invalid header length`() {
        ...
        val invalidHeaderLength = byteArrayOf(
            0x4D, 0x54, 0x68, 0x64, // "MThd"
            0x00, 0x00, 0x00, 0x08, // Wrong header length (8 instead of 6)
            0x00, 0x00, // Format 0
            0x00, 0x01, // 1 track
            0x00, 0x60, // 96 ticks per quarter note
            0x00, 0x00, // Extra bytes due to wrong length
        )
    ...
    fun `should handle truncated MIDI file gracefully`() {
        ...
        val truncatedMidi = byteArrayOf(
            0x4D, 0x54, 0x68, // Incomplete "MThd" header
        )
    ...
    fun `should handle MIDI file with single note`() {
        ...
        val singleNoteMidi = byteArrayOf(
            // Header chunk
            0x4D, 0x54, 0x68, 0x64, // "MThd"
            0x00, 0x00, 0x00, 0x06, // Header length
            0x00, 0x00, // Format 0
            ...
            // Track chunk
            0x4D, 0x54, 0x72, 0x6B, // "MTrk"
            0x00, 0x00, 0x00, 0x0A, // Track length (10 bytes)

            // Note on event
            0x00, // Delta time 0
            0x90.toByte(), 0x3C, 0x40, // Note on, channel 0, pitch 60 (C4), velocity 64

            // Note off event
            0x60, // Delta time 96 (1 beat at 96 ticks per beat)
            0x80.toByte(), 0x3C, 0x40, // Note off, channel 0, pitch 60, velocity 64
    ...
    fun `should handle invalid track chunk ID`() {
        ...
        val invalidTrackMidi = byteArrayOf(
            // Valid header
            0x4D, 0x54, 0x68, 0x64, // "MThd"
            0x00, 0x00, 0x00, 0x06, // Header length
            0x00, 0x00, // Format 0
            ...
            // Invalid track chunk
            0x4D, 0x58, 0x72, 0x6B, // "MXrk" instead of "MTrk"
            0x00, 0x00, 0x00, 0x04, // Track length
            0x00, 0xFF.toByte(), 0x2F, 0x00, // End of track
        )
    ...
    fun `should handle note on with zero velocity as note off`() {
        ...
        val noteOnZeroVelocityMidi = byteArrayOf(
            // Header chunk
            0x4D, 0x54, 0x68, 0x64, // "MThd"
            0x00, 0x00, 0x00, 0x06, // Header length
            0x00, 0x00, // Format 0
            ...
            // Note on event
            0x00, // Delta time 0
            0x90.toByte(), 0x3C, 0x40, // Note on, channel 0, pitch 60, velocity 64

            // Note on with zero velocity (equivalent to note off)
            0x60, // Delta time 96
            0x90.toByte(), 0x3C, 0x00, // Note on, channel 0, pitch 60, velocity 0
    ...
    fun `should handle tempo change events correctly`() {
        ...
        val tempoChangeMidi = byteArrayOf(
            // Header chunk
            0x4D, 0x54, 0x68, 0x64, // "MThd"
            0x00, 0x00, 0x00, 0x06, // Header length
            0x00, 0x00, // Format 0
            ...
            0x00, // Delta time 0
            0xFF.toByte(), 0x51, 0x03, // Meta event, tempo change, length 3
            0x07, 0xA1.toByte(), 0x20, // 500,000 microseconds per beat
    ...
    fun `should handle overlapping notes on same channel and pitch`() {
        ...
        val overlappingNotesMidi = byteArrayOf(
            // Header chunk
            0x4D, 0x54, 0x68, 0x64, // "MThd"
            0x00, 0x00, 0x00, 0x06, // Header length
            0x00, 0x00, // Format 0
            ...
            0x30, // Delta time 48
            0x90.toByte(), 0x3C, 0x50, // Note on, channel 0, pitch 60, velocity 80

            // Note off
            0x30, // Delta time 48
            0x80.toByte(), 0x3C, 0x40, // Note off, channel 0, pitch 60
    ...
    fun `should handle SMPTE time division format`() {
        ...
        val smpteMidi = byteArrayOf(
            // Header chunk
            0x4D, 0x54, 0x68, 0x64, // "MThd"
            0x00, 0x00, 0x00, 0x06, // Header length
            0x00, 0x00, // Format 0
            0x00, 0x01, // 1 track
            0xE7.toByte(), 0x28, // SMPTE format: -25 frames/sec, 40 ticks/frame
    ...
    @Test
    fun `should parse resource file successfully when mocked`() = runTest {
        // GIVEN: Mocked MidiResources
        mockkObject(MidiResources)
        try {
            val mockMidiBytes = byteArrayOf(
                // Minimal valid MIDI
                0x4D, 0x54, 0x68, 0x64, // "MThd"
                0x00, 0x00, 0x00, 0x06, // Header length
                0x00, 0x00, // Format 0
    ...
    @Test
    fun `should propagate exception when resource loading fails`() = runTest {
    ...
    fun `should handle multiple tracks correctly`() {
        ...
        val multiTrackMidi = byteArrayOf(
            // Header chunk
            0x4D, 0x54, 0x68, 0x64, // "MThd"
            0x00, 0x00, 0x00, 0x06, // Header length
    ...
    fun `should skip unknown meta events correctly`() {
        ...
        val unknownMetaMidi = byteArrayOf(
            // Header chunk
            0x4D, 0x54, 0x68, 0x64, // "MThd"
            0x00, 0x00, 0x00, 0x06, // Header length
            0x00, 0x00, // Format 0
            ...
            // Unknown meta event
            0x00, // Delta time 0
            0xFF.toByte(), 0x7F, 0x03, // Meta event, unknown type 0x7F, length 3
            0x01, 0x02, 0x03, // Arbitrary data
