...
class WaveEventSoundIntegrationTest : KoinTest {
     ...
     */
    @Test
    fun `should play sound during wave events with correct parameters`() =
        runTest {
            ...
            if (frequencySlot.isCaptured) {
                ...
                println(
                    "✅ Sound playback test - Frequency: ${frequencySlot.captured}Hz, Amplitude: ${amplitudeSlot.captured}, Duration: ${durationSlot.captured}",
                )
            }
     ...
     */
    @Test
    fun `should maintain precise audio timing synchronization`() =
        runTest {
            val waveStartTime = Instant.fromEpochMilliseconds(1000)
            val timingTestPoints =
                listOf(
     ...
     */
    @Test
    fun `should handle platform-specific audio implementations correctly`() =
        runTest {
     ...
     */
    @Test
    fun `should handle audio system errors gracefully during wave events`() =
        runTest {
            val waveStartTime = Instant.fromEpochMilliseconds(1000)
                    ...
                    when (exception) {
                        ...
                        is IllegalArgumentException ->
                            assertTrue(
                                ...
                                "Should propagate IllegalArgumentException",
                            )
                    }
                }
     ...
     */
    @Test
    fun `should meet audio latency and performance requirements`() =
        runTest {
            val waveStartTime = Instant.fromEpochMilliseconds(1000)
            val iterations = 20
            ...
            val p95Latency =
                latencyMeasurements.sorted().let { sorted ->
     ...
     */
    @Test
    fun `should maintain consistent audio buffer behavior across platforms`() =
        runTest {
            ...
            sampleRates.forEach { sampleRate ->
                try {
                    ...
                    println(
                        "✅ Audio buffer test - Sample rate: ${sampleRate}Hz, Samples: ${audioBuffer.sampleCount}, Buffer size: ${audioBuffer.getRawBuffer().size} bytes",
                    )
                } catch (e: Exception) {
     ...
     */
    @Test
    fun `should manage sound system resources correctly`() =
        runTest {
            val waveStartTime = Instant.fromEpochMilliseconds(1000)
