...
class EventsViewModelIOSTest {
    @Test
    fun `EventsViewModel initializes with empty state`() =
        runTest {
    ...
    @Test
    fun `filterEvents with onlyFavorites works correctly`() =
        runTest {
            val viewModel = createTestEventsViewModel()
    ...
    @Test
    fun `filterEvents with onlyDownloaded works correctly`() =
        runTest {
            val viewModel = createTestEventsViewModel()
    ...
    @Test
    fun `filterEvents with both flags works correctly`() =
        runTest {
            val viewModel = createTestEventsViewModel()
    ...
    private class MockFilterEventsUseCase : FilterEventsUseCase {
        override fun invoke(
            ...
            criteria: com.worldwidewaves.shared.domain.usecases.EventFilterCriteria,
        ): List<IWWWEvent> = emptyList()
    }
